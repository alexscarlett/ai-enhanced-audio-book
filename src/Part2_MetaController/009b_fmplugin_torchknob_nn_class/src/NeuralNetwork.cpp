    ...
    : n_inputs{_n_inputs}, n_outputs{_n_outputs}
{
    ...
    softmax = register_module(
        "softmax",
        torch::nn::Softmax(1)); // Normalizes the values in each row of a tensor into a probability distribution, where each element represents the probability of that element being chosen from the row
}
...
torch::Tensor NeuralNetwork::forward(const torch::Tensor &input)
{
    ...
    out = softmax(out); // see comment in constructor
