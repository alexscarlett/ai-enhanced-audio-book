...
{
  // better to check shape and crash here than later with a cryptic error
  assert(inputs.size() == (unsigned long)n_inputs);
  assert(outputs.size() == (unsigned long)n_outputs);

  torch::Tensor inT = torch::from_blob(
                          // get access to the raw memory storing vector of floats
                          (float *)(inputs.data()), inputs.size())
...
// This algorithm was taken from the Wekinator systems
