...

{
    ...
    auto logFile = juce::File::getSpecialLocation(juce::File::userDocumentsDirectory)
                       .getChildFile("MyPluginLog.txt");
    juce::Logger::setCurrentLogger(new MyLogger(logFile));

    // Now DBG() will log to file too
    DBG("Plugin started");
    juce::Logger::writeToLog("Plugin started");
...
{
    juce::Logger::writeToLog("handleNoteOn - midi note" + juce::String(midiNoteNumber));
